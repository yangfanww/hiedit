...
class HIEDITMM(BaseEditor):
    ...
    def train(self, loader: DataLoader, mode: str = None):
        """
        The training method for HiEditMM.
        """
        ...
        for tuples in tqdm(limited_loader, desc=f"Train", ncols=100, total=max_steps):
            ...
            self.update_hypernet(param_shifts, update=False, mask=mask)
            swanlab.log({
    ...
    def predict_param_shifts(self) -> Dict[str, torch.FloatTensor]:
        ...
        for module_idx, module_name in enumerate(self.config.model.edit_modules):
            ...
            keys = torch.cat([
                torch.load(f"{self.config.editor.cache_dir}/{self.config.model.name}_{self.config.editor.name}_{self.config.dataset.name}_{self.config.num_seq}_k{self.config.editor.n_layers}/{module_idx}_{idx}_keys.pth")
                for idx in range(math.ceil(self.config.dataset.n_edits / self.config.dataset.batch_size))
            ])
            values_grad = torch.cat([
                torch.load(f"{self.config.editor.cache_dir}/{self.config.model.name}_{self.config.editor.name}_{self.config.dataset.name}_{self.config.num_seq}_k{self.config.editor.n_layers}/{module_idx}_{idx}_values_grad.pth")
                for idx in range(math.ceil(self.config.dataset.n_edits / self.config.dataset.batch_size))
            ])
        ...
        for module_idx, module_name in enumerate(self.config.model.edit_modules):
            ...
            param_shift = torch.linalg.solve(mat, keys.T @ value_diffs) * mask[module_idx]
            param_shifts[module_name] = param_shift.to(next(self.model.parameters()).device)

        return param_shifts, mask
        
        
    def update_hypernet(self, param_shifts: Dict[str, torch.FloatTensor], update: bool, mask):
    ...
    def run(self, train_loader: DataLoader, valid_loader: DataLoader):
        for epoch in tqdm(range(self.config.editor.n_epochs), desc="epoch"):
            ...
            # self.hi_net.load_state_dict(torch.load("checkpoints/cache_k6_llama-3_hieditmm_zsre_2000_0_hi_net.pth"))
            # self.net.load_state_dict(torch.load("checkpoints/cache_k6_llama-3_hieditmm_zsre_2000_0_low_net.pth"))
